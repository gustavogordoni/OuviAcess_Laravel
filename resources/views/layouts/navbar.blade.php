      ...
      <div class="offcanvas-body">
        <ul class="navbar-nav justify-content-center flex-grow-1 nav-pills">
          ...
          <li class="nav-item mx-2">
            <a href="{{ route('index') }}" class="nav-link @php echo ativar('/') @endphp text-center">
          ...
          <li class="nav-item mx-2">
            <a href="{{ route('history') }}" class="nav-link @php echo ativar('history'); echo history('history'); echo history('edit-request'); echo history('show-request') @endphp text-center">
              <svg xmlns="http://www.w3.org/2000/svg" class="bi d-block mx-auto mb-1" width="15" height="15" fill="currentColor" viewBox="0 0 16 16">
          ...
          @if (Str::contains(request()->url(), 'show-request'))        
        ...
        <div class="d-flex my-auto justify-content-evenly">
          ...
          <form action="{{ route('theme') }}" method="POST" class="my-auto">
            @csrf
            <button value="{{ $value }}" type="submit" class="btn {{ $class }} mx-1 rounded-circle p-2" id="alterar_tema" name="theme">{!! $Dark_Light !!}</button>
